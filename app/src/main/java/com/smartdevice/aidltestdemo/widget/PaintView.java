package com.smartdevice.aidltestdemo.widget;
...
public class PaintView extends View {
   ...
   final public static int MODE_CROP = 6;
   final public static int MODE_SLIDE = 7;
   final public static int MODE_ERASER_ALL = 8;
   ...
   protected void onSizeChanged(int w, int h, int oldw, int oldh) {
      ...
      if (w > 0 && h > 0 && w != mBmpWidth && h != mBmpHeight) {
         ...
         if (mCanvasBmp == null || mCanvasBmp.isRecycled()) {
            try {
               mCanvasBmp = Bitmap.createBitmap(mBmpWidth, mBmpHeight,
            ...
            Bitmap sbmp = null;
            try {
               sbmp = Bitmap.createBitmap(mBmpWidth, mBmpHeight,
                     Bitmap.Config.ARGB_8888);
            } catch (OutOfMemoryError e) {
            }
            if (sbmp != null) {
   ...
   // BoardView.this.getWidth(),
   // BoardView.this.getHeight());
   ...
   public void clearCanvas(boolean bNeedShare) {
   ...
   public void setDrawMode(int mode) {
      ...
      if (mode == MODE_ERASER_NORMAL || mode == MODE_ERASER_LINE) {
         ...
         mPaint.setStrokeWidth(getEraseWidth());
      } else if (mode == MODE_DRAW_CURVE) {
   ...
   public void undo() {
      ...
      Message msg = new Message();
      msg.what = 101;
      msg.arg1 = 1;// hide undo
      for (int i = mPathList.size() - 1; i >= 0; i--) {
         if (!mPathList.get(i).bRemove && mPathList.get(i).bShow) {
            if (!undoOK) {
               mPathList.get(i).bShow = false;
               undoOK = true;

            } else {
               msg.arg1 = 2;// show redo
               break;
            }
         }
      }
   ...
   public void setHighliterColor(int color) {
      mPaint.setColor(color);
      highliterColor = color;
   }
   ...
   public void setHighliterWidth(int width) {
      mPaint.setStrokeWidth(width);
      highliterWidth = width;
   }
   ...
   /**
    * 是否进行过签名。不校验签名内容，画板上有内容就算进行过签名了。
    * 
    * @return
    */
